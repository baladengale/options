...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Call the derivatives anomaly skill backend.",
    )
    parser.add_argument("stock_symbol", help="Stock symbol, such as HK.00700 or US.AAPL")
    parser.add_argument("--time-range", type=int, default=7, help="Natural day window, default 7")
    parser.add_argument(
        "--analysis-dimensions",
        nargs="*",
        help="Analysis dimensions, supports space-separated or comma-separated values",
    )
    parser.add_argument(
        "--language-id",
        type=int,
...
def main():
    ...
    analysis_dimensions = parse_list_args(args.analysis_dimensions)
    ...
    if ret != RET_OK:
        print(f"get_derivative_unusual error: {data}", file=sys.stderr)
        return 1
